...
sat_patches = {
    #'south': [-60, -30, 75, -60],
    'south': [-50, -30, 90, -50, 0.001],  # The south patch is the primary patch
    #'north': [150, 15, 195, -15]
    #'north': [120, 10, 195, -10]
    #'north': [120, 0, 195, -20, 1000],  # The Northern patch is secondary
}
...
with open('patches_sat.txt', 'w') as fout:
    ...
    for name in sat_patches:
        lonmin, latmax, lonmax, latmin, priority = sat_patches[name]
        fout.write('--patch\n')
    ...
    while dec_start < dec_max:
        ...
        for ilon in range(nstep):
            ...
            # see if the tile overlaps with the SAT patches
            overlaps = False
            for patch in sat_patches:
                lonmin, latmax, lonmax, latmin, priority = sat_patches[patch]
...
plot_patches(sat_patches)
plt.savefig('priority_sat.png')
